...
Where $\sigma$ is the logistic sigmoid function, $\boldsymbol W$ is the recurrent weight matrix and $I$ is a projection matrix. Stacking RNNs, such that the hidden vector $\boldysmbol h$ of a lower layer becomes the input of an upper layer is one method for creating deeper architectures [?]
...
\textbf{Questions}
How do we normalize over time?
Does batch normalization allow for a higher learning rate?
